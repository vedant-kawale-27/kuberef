...
def test_get_configmap_refs():
    """Verify that we extract configmap references from Pod specs correctly."""
    ...
    assert "my-cm" in refs
    assert "key1" in refs["my-cm"]
    assert "env-cm" in refs
    assert refs["env-cm"] == set()


def test_configmap_validation_integration(tmp_path):
    """Integration test verifying ConfigMap validation with live K8s API mock."""
    from unittest.mock import patch, MagicMock
    from typer.testing import CliRunner
    from kuberef.main import app
  ...
  - name: app
    ...
    envFrom:
      ...
      - configMapRef:
          name: missing-env-cm
"""
    manifest_file = tmp_path / "configmap-test.yaml"
    manifest_file.write_text(yaml_content)
    ...
    with patch("kuberef.main.config.load_kube_config") as mock_load, \
        ...
        def mock_read_config_map(name, namespace=None):
            cm_data = {
                "valid-cm": {"valid-key": "some-value"},
                "env-cm": {"another-key": "another-value"}
            }
            if name in cm_data:
                cm = MagicMock()
                cm.data = cm_data[name]
                cm.binary_data = {}
                return cm
            from kubernetes.client.rest import ApiException
            raise ApiException(status=404, reason="Not Found")
            
        mock_api.read_namespaced_config_map.side_effect = mock_read_config_map
        # Secrets mock
        mock_api.read_namespaced_secret.side_effect = Exception("Should not call secret endpoint")

        result = runner.invoke(app, [str(manifest_file)])
        
        # Expected output should show table with Type, Resource Name and Status
        assert result.exit_code == 1
        assert "Security Audit: configmap-test.yaml" in result.output
        assert "CONFIGMAP" in result.output
        assert "valid-cm" in result.output
        assert "missing-cm" in result.output
        assert "env-cm" in result.output
        assert "missing-env-cm" in result.output
        
        # Verify status rows in CLI output
        assert "KEY MISSING: missing-key" in result.output
        assert "FAIL (ConfigMap Missing)" in result.output
        assert "PASS" in result.output
from kuberef.main import get_secret_refs, get_yaml_files
